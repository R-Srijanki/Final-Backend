...
export async function registerUser(req, res) {
  try {
    ...
    if (!fullName || !username || !email || !password)
      //if any fields data missing return
    ...
    return res
      .status(500)
      .json({ "error occured during registering user": err.message });
  }
}
...
export async function loginUser(req, res) {
  try {
    ...
    if (!username || !email || !password)
      //if any data missing then return
      return res.status(400).json({ message: "Missing fields" });
    //find user with given details
    const exists = await User.findOne({ email, username }).populate(
      "channel",
      "handle"
    );
    if (!exists)
      //if user is not registered then return
      return res.status(401).json({ message: "Invalid email or username" });
    //compare hashed password with typed password
    let validPassword = bcrypt.compareSync(password, exists.password);
    if (!validPassword) {
      //if not matched then wrong password
